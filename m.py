...
TOKEN = '7526972070:AAFQ7dPjNfxU4nZLU5JSRewyfYYArqDpDJI'
...
def start(message):
 ...
 #       types.KeyboardButton('/Show_user_commands'),
  #      types.KeyboardButton('/Show_all_approved_users')
    ...
    bot.send_message(message.chat.id, "👋 Welcome TO V3 WORLD 🗿:", reply_markup=markup)
...
def attack(message):
    ...
    if user_id not in AUTHORIZED_USERS:
        bot.send_message(message.chat.id, "🚫 Access Denied! Contact the owner for assistance: @kaushik_oo7")
        return
...
def owner(message):
    ...
    bot.send_message(message.chat.id, "📞 Contact the owner: @kaushik_oo7")
